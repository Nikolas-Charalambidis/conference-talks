...
- **Why** - *"Fear of missing out"* leads us, so we should forget all of that, release the fear, and think of the real reasons:
  - **For the business**
    - Some technologies gain popularity and some eventually die off, which is good news as we don't need to learn all technologies in order to make a decision on which ones to use going forward. 
  ...
  - **For the team**
    - Usually, releases are painful and lengthy, and introducing correct tools and processes reduces the time and manual work, development costs, and team frustration - it is also about mindset.
    ...
    - *Approach*: Build spikes/prototypes and start using non-production code first (build, deploy, CI/CD, test) such as internal tools or a skunkwork projects (hack days) that might or might not prove valuable in time (developer plugins, new products...).
  - **For you**
    ...
    - *Approaches*: Blogs, tutorials & videos, online events & streams, user groups, online courses, books, pet projects, join an opensource project, find a project at work or get a new job.
- **Developers and business needs don't match**, the approaches for the business and the team overlap themselves but not for ourselves, because we want to learn a new technology personally and not to build a new production service, so how to bridge the gap?
...
- **Rule 20% of time** says we should have dedicated 20% of our working time to self-education where developers don't do what business wants them to do, but there is a lot of value to it because developers will be doing something that might have value to the business later or might have a value to the team because we can work on things like continuous delivery or deployment pipeline.
  - **Risks**: Some organizations are not necessarily sold on investing this sort of time in these activities.
    ...
    - A higher risk is not giving them training and space to grow and learn, they will definitely leave anyway.
  - **Benefits**:
